...
// Страница детальной информации о товаре для поставщика
...
if (!$supplierId) {
    ...
    if ($productId <= 0) {
        $error = "Ошибка: неверный идентификатор товара.";
    } else {
        ...
        if (!$productDetails) {
            ...
            // Получаем историю транзакций по этому товару
            $productTransactions = $warehouseController->getProductTransactions($productId, 10);
            
            // Фильтруем транзакции, относящиеся к заказам этого поставщика
            $supplierTransactions = array_filter($productTransactions, function($transaction) use ($supplierId) {
                if ($transaction['reference_type'] === 'order' && $transaction['reference_id'] > 0) {
                    // Проверяем, принадлежит ли заказ этому поставщику
            ...
            // Получаем последнюю дату поставки этого товара этим поставщиком
            $lastDeliveryDate = $supplierController->getProductLastDelivery($supplierId, $productId);
            
            // Получаем общее количество поставленного товара
            $totalSupplied = 0;
...
// Проверяем если метод запроса POST и была нажата кнопка отправки предложения
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['send_offer'])) {
    // Отправка сообщения с предложением о поставке
    ...
    if ($recipient) {
        $subject = "Предложение о поставке: " . $productDetails['name'];
        $content = "Здравствуйте!\n\n";
        $content .= "Я хотел бы предложить поставку товара \"" . $productDetails['name'] . "\".\n\n";
        $content .= "Количество: " . $_POST['offer_quantity'] . " " . $productDetails['unit'] . "\n";
        $content .= "Цена за единицу: " . $_POST['offer_price'] . " ₴\n\n";
        ...
        if ($result['success']) {
            $message = "Ваше предложение успешно отправлено менеджеру по закупкам!";
        } else {
            $error = "Ошибка при отправке предложения: " . $result['message'];
        }
    } else {
        $error = "Ошибка: не найден менеджер по закупкам для отправки предложения.";
    }
}
?>
...
<head>
    ...
    <title>Детали товара - Винное производство</title>
...
<body class="bg-gray-100 min-h-screen">
    ...
    <div class="container mx-auto mt-6 px-4">
        <div class="bg-red-100 border-l-4 border-red-500 text-red-700 p-4" role="alert">
            <p><?php echo $error; ?></p>
            <p class="mt-2">
                <a href="products.php" class="text-red-700 underline">Вернуться к списку товаров</a>
            </p>
        </div>
    </div>
    <?php else: ?>
    ...
    <div class="container mx-auto flex flex-wrap mt-6 px-4">
        ...
        <aside class="w-full md:w-1/4 pr-0 md:pr-6">
            ...
            <div class="bg-white rounded-lg shadow-md p-4 mb-6">
                <h3 class="font-semibold text-lg mb-3">Информация о товаре</h3>
                <div class="space-y-3">
                    ...
                    <div class="flex justify-between">
                        <span class="text-gray-600">Текущий запас:</span>
                    ...
                    <div class="flex justify-between">
                        <span class="text-gray-600">Мин. запас:</span>
                    ...
                    <div class="flex justify-between">
                        <span class="text-gray-600">Последняя поставка:</span>
                        <span class="font-medium">
                            <?php echo $lastDeliveryDate ? date('d.m.Y', strtotime($lastDeliveryDate)) : 'Нет данных'; ?>
                        </span>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-lg shadow-md p-4">
                ...
                <div class="space-y-2">
                    <button id="showOfferForm" class="w-full flex justify-center items-center px-4 py-2 bg-amber-600 text-white rounded-md hover:bg-amber-700">
                        <i class="fas fa-file-invoice-dollar mr-2"></i> Создать предложение
                    </button>
        ...
        <main class="w-full md:w-3/4">
            <div class="bg-white rounded-lg shadow-md p-6 mb-6">
                ...
                <?php if (isset($error) && $error && $error !== "Ошибка: данные поставщика не найдены. Обратитесь к администратору."): ?>
                <div class="bg-red-100 border-l-4 border-red-500 text-red-700 p-4 mb-6" role="alert">
                    <p><?php echo $error; ?></p>
                </div>
                <?php endif; ?>
                
                <!-- Форма создания предложения (скрыта по умолчанию) -->
                <div id="offerForm" class="hidden bg-amber-50 p-6 rounded-lg mb-6">
                    <h3 class="text-lg font-medium text-amber-800 mb-4">Создание предложения о поставке</h3>
                    <form method="POST" action="">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                <label for="offer_quantity" class="block text-sm font-medium text-gray-700 mb-1">Предлагаемое количество *</label>
                                <div class="flex">
                            ...
                            <div>
                                <label for="offer_price" class="block text-sm font-medium text-gray-700 mb-1">Цена за единицу *</label>
                                <div class="flex">
                            ...
                            <div class="md:col-span-2">
                                <label for="offer_notes" class="block text-sm font-medium text-gray-700 mb-1">Дополнительная информация</label>
                                <textarea id="offer_notes" name="offer_notes" rows="4" 
                                          ...
                                          placeholder="Укажите сроки поставки, особые условия или другую важную информацию"></textarea>
                            </div>
                        </div>
                ...
                <div class="mb-6">
                    ...
                    <div class="bg-gray-50 p-4 rounded-lg">
                        <p class="text-gray-700">
                            <?php echo empty($productDetails['description']) ? 'Описание отсутствует.' : nl2br(htmlspecialchars($productDetails['description'])); ?>
                        </p>
                    </div>
                </div>
                ...
                <!-- История поставок -->
                <div>
                    <h3 class="text-lg font-medium text-gray-900 mb-3">История поставок</h3>
                    <?php if (empty($supplierTransactions)): ?>
                    <div class="bg-gray-50 p-6 rounded-lg text-center">
                        <p class="text-gray-500">У вас пока нет истории поставок этого товара.</p>
                        <button id="createFirstOffer" class="mt-4 inline-flex items-center px-4 py-2 bg-amber-600 text-white rounded-md hover:bg-amber-700">
                            <i class="fas fa-plus mr-2"></i> Создать первое предложение
                        </button>
                    </div>
                    <?php else: ?>
                                ...
                                <tr>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                        <a href="order_details.php?id=<?php echo $transaction['reference_id']; ?>" class="text-amber-600 hover:text-amber-800">
                                            Заказ #<?php echo $transaction['reference_id']; ?>
                                        </a>
                                    </td>
                ...
                <div class="mt-6 pt-6 border-t border-gray-200">
                    <h3 class="text-lg font-medium text-gray-900 mb-3">Рекомендации по поставкам</h3>
                    ...
                    <div class="bg-amber-50 p-4 rounded-lg">
                        ...
                        <div class="flex items-start">
                            ...
                            <div class="ml-3">
                                <h4 class="text-sm font-medium text-amber-800">Рекомендуется поставка</h4>
                                <p class="mt-1 text-sm text-amber-700">
                                    Текущий запас этого товара ниже минимального порога. Рекомендуем предложить поставку в ближайшее время.
                                    Рекомендуемое количество: <?php echo max(($productDetails['min_stock'] * 2) - $productDetails['quantity'], 1) . ' ' . $productDetails['unit']; ?>
                                </p>
                                <div class="mt-2">
                                    <button id="recommendedOffer" class="inline-flex items-center px-3 py-1.5 border border-transparent text-xs font-medium rounded-md text-amber-700 bg-amber-100 hover:bg-amber-200">
                                        Создать рекомендуемое предложение
                                    </button>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="flex items-start">
                            ...
                            <div class="ml-3">
                                <h4 class="text-sm font-medium text-amber-800">Достаточный запас</h4>
                                <p class="mt-1 text-sm text-amber-700">
                                    Текущий запас этого товара выше минимального порога. Поставка не требуется в настоящее время, но вы всегда
                                    можете предложить выгодные условия для будущих поставок.
                                </p>
                            </div>
                        </div>
                        <?php endif; ?>
                ...
                <!-- График последних поставок -->
                <?php if (!empty($supplierTransactions)): ?>
                <div class="mt-6 pt-6 border-t border-gray-200">
                    <h3 class="text-lg font-medium text-gray-900 mb-3">График поставок</h3>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Кнопка "Создать первое предложение"
            ...
            // Кнопка "Создать рекомендуемое предложение"
            ...
            const chartData = {
                labels: [
                    ...
                    // Максимум 10 последних транзакций
                ...
                datasets: [{
                    label: 'Количество поставленного товара',
                    data: [
                        <?php 
                        foreach ($displayTransactions as $index => $transaction) {
                ...
                options: {
                    ...
                    scales: {
                        y: {
                            ...
                            title: {
                                display: true,
                                text: 'Количество (<?php echo htmlspecialchars($productDetails['unit']); ?>)'
                            }
                        },
                        x: {
                            title: {
